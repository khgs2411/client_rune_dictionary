...
## Flow Framework (Project Management)

**CRITICAL**: Delegate ALL Flow operations to the Flow sub-agent via `Task tool → subagent_type: "flow"`.

**DO NOT** use Skill/SlashCommand with flow-\* commands or read .flow/ files directly.

**Delegate when user mentions**: status, progress, tasks, phases, iterations, DASHBOARD, PLAN, brainstorm, or any /flow-\* command.

**Role separation**: Flow agent = PROJECT MANAGER | You = ENGINEER (code, debugging, git)
