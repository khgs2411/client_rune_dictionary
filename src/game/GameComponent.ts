...
export abstract class GameComponent implements I_GameComponent {
	 ...
	 */
	protected getComponent<T extends GameComponent>(componentClass: new (...args: any[]) => T): T | null {
	 ...
	 */
	protected requireComponent<T extends GameComponent>(componentClass: new (...args: any[]) => T): T {
		const component = this.getComponent(componentClass);
		if (!component) {
			throw new Error(`[${this.constructor.name}] Required component ${componentClass.name} not found on GameObject "${this.gameObject.id}"`);
		}
		return component;
	}
	 ...
	 */
	protected restrictComponent(componentClass: new (...args: any[]) => GameComponent, reason?: string): void {
