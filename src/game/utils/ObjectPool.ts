...
export class ObjectPool<T extends GameObject = GameObject> {
	...
	acquire(config: any = {}): T {
		...
		if (this.available.length > 0) {
			...
			console.log(`✨ [ObjectPool:${this.type}] Created new instance (active: ${this.active.size + 1})`);
		}
	...
	release(obj: T): void {
		if (!this.active.has(obj.id)) {
			console.warn(`[ObjectPool:${this.type}] Attempted to release object that wasn't active: ${obj.id}`);
			return;
		}
		...
		if (this.maxSize === 0 || this.available.length < this.maxSize) {
			...
			console.log(`♻️  [ObjectPool:${this.type}] Returned to pool (available: ${this.available.length})`);
		} else {
			// Pool is full, actually destroy the object
			obj.destroy();
			console.log(`🗑️  [ObjectPool:${this.type}] Pool full, destroyed object (max: ${this.maxSize})`);
		}
	}
	...
	preWarm(count: number): void {
		...
		console.log(`🔥 [ObjectPool:${this.type}] Pre-warmed pool with ${toCreate} instances (total available: ${this.available.length})`);
	}
