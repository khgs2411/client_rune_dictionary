		...
		this.unregister = interaction.registerDrag(
			...
			meshComp.mesh,
			{
				...
				onEnd: (endPos) => {
					...
					console.log(`📦 [DragComponent] Drag ended for "${this.gameObject.id}" at [${endPos.x.toFixed(2)}, ${endPos.y.toFixed(2)}, ${endPos.z.toFixed(2)}]`);
			...
			},
			{
				...
				snapToGrid: this.config.snapToGrid !== undefined ? this.config.snapToGrid : () => gameConfig.editor.snapToGrid,
			},
		);
	}
