    ...
    this.unregister = interaction.registerMouseClick(
      ...
      (event, intersection) => {
        ...
        if (this.config.cameraShake) {
          vfx.shakeCamera(
            this.config.cameraShake.intensity ?? 0.1,
            this.config.cameraShake.duration ?? 0.3,
          );
        }
        ...
        if (this.config.particles) {
          vfx.spawnParticles(
            intersection.point,
            this.config.particles.count ?? 20,
            this.config.particles.color ?? 0xffd700,
            this.config.particles.speed ?? 3,
          );
        }
