...
export class KinematicCollisionComponent extends CollisionComponent {
	...
	async init(context: I_SceneContext): Promise<void> {
		...
		if (!physics.isReady()) {
			console.warn(`[KinematicPhysicsComponent] Physics not ready for GameObject "${this.gameObject.id}". Skipping.`);
			return;
		}
		...
		if (!mesh) {
			throw new Error(`[KinematicPhysicsComponent] GameObject "${this.gameObject.id}" getMesh() returned null/undefined`);
		}
