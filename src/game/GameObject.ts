...
export class GameObject {
	...
	public addComponent(component: GameComponent): this {
		if (this.isInitialized) {
			console.warn(`[GameObject] Adding component ${component.constructor.name} to already initialized GameObject "${this.id}". Component will not be initialized.`);
		}
	...
	requireByTrait<T>(trait: TraitKey): T {
		...
		if (!component) {
			throw new Error(`[GameObject] No component with trait ${trait.description} found on "${this.id}"`);
		}
		return component;
	}
	...
	public async init(context: I_SceneContext): Promise<void> {
		...
		const sortedComponents = Array.from(this.components.values()).sort((a, b) => a.priority - b.priority);
