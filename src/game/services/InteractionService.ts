...
export class InteractionService extends SceneService implements I_SceneService {
	 ...
	 * ```
	 */
	public registerKeyPress(id: string, key: string, callback: KeyPressCallback, modifiers?: { ctrl?: boolean; shift?: boolean; alt?: boolean }): () => void {
	...
	private handleClick(): void {
		...
		this.mouseClickHandlers.forEach((handler) => {
			...
			if (handler.object3D && this.context.camera) {
				...
				const intersects = this.raycast.fromCamera(this.mouse.normalizedPositionRef, this.context.camera.instance, [handler.object3D]);
	...
	private updateHover(): void {
		...
		const intersects = this.raycast.fromCamera(this.mouse.normalizedPositionRef, this.context.camera.instance, this.hoverObjectsArray);
	...
	private handleDrag(): void {
		...
		if (handler.snapToGrid) {
			const snapValue = typeof handler.snapToGrid === "function" ? handler.snapToGrid() : handler.snapToGrid;
