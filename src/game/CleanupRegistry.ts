...
export class CleanupRegistry {
	...
	cleanup(scene: Scene): void {
		...
		if (objectCount > 0 || disposableCount > 0) {
			console.log(`🧹 [CleanupRegistry] Cleaned up ${objectCount} objects, ${disposableCount} disposables${errorCount > 0 ? `, ${errorCount} errors` : ""}`);
		}
	}
	...
	private disposeTextures(material: Material): void {
		...
		const textureProps: string[] = ["map", "normalMap", "bumpMap", "displacementMap", "roughnessMap", "metalnessMap", "aoMap", "emissiveMap", "lightMap", "envMap", "alphaMap"];
