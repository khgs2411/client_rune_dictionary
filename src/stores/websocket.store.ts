...
export type WebSocketStatus = "disconnected" | "handshaking" | "connecting" | "connected" | "reconnecting";
...
export const useWebSocketStore = defineStore("websocket", () => {
	...
	const isConnecting = computed(() => status.value === "connecting" || status.value === "handshaking" || status.value === "reconnecting");
	...
	function setClientData(data: I_ClientData) {
		if (clientData.value && (clientData.value.id !== data.id || clientData.value.name !== data.name)) {
			console.warn(`[WS] Updating client data from ${clientData.value.name} (${clientData.value.id}) ` + `to ${data.name} (${data.id})`);
		}
