 ...
 */
export function useCameraRotation(angleH: Ref<number>, angleV: Ref<number>, sensitivity = { h: 0.005, v: 0.005 }): CameraRotation {
	...
	function updateVertical(delta: number) {
		angleV.value = Math.max(MIN_VERTICAL, Math.min(MAX_VERTICAL, angleV.value + delta * sensitivity.v));
	}
