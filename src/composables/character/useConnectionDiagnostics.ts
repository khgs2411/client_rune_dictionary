 ...
 */
export const testWebSocketConnection = async (url: string, protocol?: string, timeoutMs = 5000): Promise<DiagnosticResult> => {
	return new Promise((resolve) => {
		try {
...
export function useConnectionDiagnostics() {
	 ...
	 */
	const runDiagnostics = async (options?: { wsUrl?: string; protocol?: string; testPopups?: boolean }): Promise<DiagnosticResults> => {
		loading.value = true;
