<template>
  <div class="relative min-h-svh w-full overflow-hidden font-sans">
    ...
    <div v-if="isDev" class="absolute top-4 right-4 z-50">
      <ButtonGroup>
        <Button variant="outline" class="bg-background/80 backdrop-blur-md"
          @click="switchUser($event, 'tal')">Tal</Button>
        <Button variant="outline" class="bg-background/80 backdrop-blur-md"
          @click="switchUser($event, 'yazin')">Yazin</Button>
      </ButtonGroup>
    </div>
    ...
    <div class="relative z-10 flex min-h-svh items-center justify-center px-4 py-8">
      <div class="w-full max-w-md animate-fade-in space-y-8">
        ...
        <!-- Footer Text -->
        <p class="text-center text-xs text-white/50 hover:text-white/80 transition-colors cursor-pointer">
