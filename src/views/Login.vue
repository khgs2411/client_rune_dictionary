<template>
	<div class="relative min-h-svh w-full overflow-hidden font-sans">
		...
		<div v-if="isDev" class="absolute top-4 right-4 z-50">
			<ButtonGroup>
				<Button variant="outline" class="bg-background/80 backdrop-blur-md" @click="switchUser($event, 'tal')">Tal</Button>
				<Button variant="outline" class="bg-background/80 backdrop-blur-md" @click="switchUser($event, 'yazin')">Yazin</Button>
			</ButtonGroup>
		</div>
		...
		<div class="relative z-10 flex min-h-svh items-center justify-center px-4 py-8">
			<div class="w-full max-w-md animate-fade-in space-y-8">
				...
				<div class="text-center space-y-2 animate-float">
					<h1 class="font-fantasy text-5xl font-bold tracking-wider text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] text-shadow-glow">RUNE RPG</h1>
				...
				<div
					...
					<div class="absolute top-0 left-0 h-8 w-8 border-t-2 border-l-2 border-yellow-400/50 rounded-tl-lg"></div>
					<div class="absolute top-0 right-0 h-8 w-8 border-t-2 border-r-2 border-yellow-400/50 rounded-tr-lg"></div>
					<div class="absolute bottom-0 left-0 h-8 w-8 border-b-2 border-l-2 border-yellow-400/50 rounded-bl-lg"></div>
					<div class="absolute bottom-0 right-0 h-8 w-8 border-b-2 border-r-2 border-yellow-400/50 rounded-br-lg"></div>
					...
					<form @submit.prevent="handleLogin" class="space-y-6 relative z-10">
						...
						<!-- Error Message -->
						<div v-if="errorMessage" class="rounded-lg bg-red-500/20 border border-red-500/30 p-3 text-sm text-red-200 text-center animate-in fade-in slide-in-from-top-2">
							{{ errorMessage }}
						</div>
						...
						<Button
							...
							:disabled="isLoading">
							<span v-if="isLoading" class="flex items-center gap-2"> <span class="animate-spin">⚔️</span> Summoning... </span>
				...
				<!-- Footer Text -->
				<p class="text-center text-xs text-white/50 hover:text-white/80 transition-colors cursor-pointer">Need an account? Contact the Guild Master</p>
			</div>
		</div>
	</div>
</template>
