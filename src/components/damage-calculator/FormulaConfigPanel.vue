      ...
      <div class="space-y-6">
        ...
        <div class="space-y-4">
          ...
          <div v-if="currentConstants" class="space-y-3">
            <div
              v-for="(value, key) in currentConstants"
              :key="key"
...
import { useDamageCalculations, type DamageFormulaType } from '@/composables/useDamageCalculations';
...
const availableFormulas = computed(() => damageCalc.availableFormulas.value);
...
const currentConstants = computed(() => {
  const allConstants = damageCalc.customConstants.value;
...
const globalConstants = computed(() => damageCalc.customConstants.value.GLOBAL);

// Methods
const updateSelectedFormula = (formula: string) => {
...
const updateConstant = (key: string, value: string | number) => {
  ...
  const formulaKey = getFormulaConstantKey(selectedFormula.value);
  if (!formulaKey) return;
  
  const newConstants = {
    ...damageCalc.customConstants.value,
    [formulaKey]: {
      ...damageCalc.customConstants.value[formulaKey],
      [key]: numValue
    }
  };
  
  damageCalc.updateConstants(newConstants);
...
const updateGlobalConstant = (key: string, value: string | number) => {
  ...
  const newConstants = {
    ...damageCalc.customConstants.value,
    GLOBAL: {
      ...damageCalc.customConstants.value.GLOBAL,
      [key]: numValue
    }
  };
...
const getFormulaConstantKey = (formula: DamageFormulaType): string | null => {
  switch (formula) {
...
const resetFormulaConstants = () => {
  ...
  const originalConstants = damageCalc.DAMAGE_CONSTANTS.value;
  const newConstants = {
    ...damageCalc.customConstants.value,
    [formulaKey]: { ...originalConstants[formulaKey] }
  };
