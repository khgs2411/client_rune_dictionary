		...
		<SheetContent class="overflow-y-auto" @wheel.stop>
			...
			<div class="py-4 space-y-6">
					...
					<div class="pt-4 border-t border-border space-y-6 first:pt-0 first:border-t-0">
						...
						<template v-for="field in group" :key="field.key">
							...
							<div v-else-if="field.type === 'number'" class="space-y-3">
								...
								<Slider
									:model-value="[field.ref[field.fieldName]]"
									@update:model-value="(val) => val && (field.ref[field.fieldName] = val[0])"
									:min="field.meta.min"
									:max="field.meta.max"
				...
				<div class="pt-4 border-t border-border space-y-4">
					...
					<div class="space-y-2">
						<Button @click="handleResetScene" variant="destructive" size="sm" class="w-full"> Reset Scene Positions </Button>
						<p class="text-xs text-muted-foreground">Clear all saved object positions and reset to defaults. Requires scene reload.</p>
					</div>
				</div>
			</div>
		</SheetContent>
