<template>
	<div v-if="config.editor.enabled" class="fixed top-20 right-4 z-40 bg-card/95 backdrop-blur-sm border border-border rounded-lg shadow-lg p-4 space-y-4 w-72" @wheel.stop>
		<!-- Header -->
		...
		<div class="space-y-3">
			...
			<div class="space-y-2">
				...
				<Slider :model-value="[config.editor.snapToGrid]" @update:model-value="(val) => val && (config.editor.snapToGrid = val[0])" :min="0.1" :max="2" :step="0.1" class="w-full" />
			...
			<div class="space-y-2">
				<div class="flex justify-between items-center">
					...
					<span class="text-xs text-muted-foreground font-mono">{{ config.editor.dragOpacity.toFixed(2) }}</span>
				</div>
				<Slider :model-value="[config.editor.dragOpacity]" @update:model-value="(val) => val && (config.editor.dragOpacity = val[0])" :min="0.1" :max="1" :step="0.05" class="w-full" />
			</div>
		</div>
		...
		<div class="pt-3 border-t border-border space-y-2">
			...
			<!-- Reset Scene Button -->
			<Button @click="handleResetScene" variant="destructive" size="sm" class="w-full text-xs"> Reset All Positions </Button>
		</div>
	</div>
</template>
