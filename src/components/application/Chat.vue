	...
	<ChatWindow :container-ref="containerRef" storageKey="chat" :minWidth="300" :minHeight="200"
		...
		<div class="chat-content">
			<div ref="messagesContainer" class="messages">
				<ChatMessage @action="handleAction" v-for="(message, index) in messages" :key="index"
					:message="message" />
			</div>
			...
			<div class="input-area">
				<div class="input-wrapper">
					<InputGroup>
						<InputGroupAddon v-if="whisperMode" class="chat-element" @click="resetChatState"
							v-tooltip="'Click to clear whisper'">
							<div class="flex align-items-center gap-2 whisper-indicator pointer">
								<i :class="'pi pi-user'"></i>
								<span>Whisper: {{ targetEntity?.name }}</span>
							</div>
						</InputGroupAddon>
						<InputText v-model="inputMessage" @update:model-value="handleMacros"
							@keyup.enter="() => sendMessage()" class="chat-element"
							:class="{ 'whisper-indicator': whisperMode }" placeholder="Enter message..." ref="inputRef"
							:disabled="status !== 'OPEN'" />
					</InputGroup>
					<Button @click="() => sendMessage()" :disabled="status !== 'OPEN' || !inputMessage.trim()"> Send
					</Button>
				</div>
			</div>
		</div>
