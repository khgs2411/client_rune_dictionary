<template>
	<div v-if="isLoading || !isComplete" class="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
		<div class="flex flex-col items-center gap-6 w-full max-w-md px-6">
			...
			<div class="w-full space-y-2">
				<div class="h-2 w-full overflow-hidden rounded-full bg-secondary">
					<div class="h-full bg-primary transition-all duration-300 ease-out" :style="{ width: loadingPercentage }" />
				</div>
			...
			<!-- Current Asset -->
			<div v-if="currentAsset" class="text-xs text-center text-muted-foreground truncate w-full">Loading: {{ currentAsset }}</div>
...
import type { SceneLoadingStartPayload, SceneLoadingProgressPayload, SceneErrorPayload, SceneLoadingEvent, SceneLoadedPayload } from "@/common/events.types";
