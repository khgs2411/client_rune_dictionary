<template>
  ...
  <div
    ...
    <div
      class="bg-card/90 backdrop-blur-sm rounded-lg border border-border shadow-lg p-3 space-y-2">
      <!-- Drag Handle -->
      <div
        ref="handle"
      ...
      <div class="grid grid-cols-4 sm:grid-cols-8 gap-1 sm:gap-2">
        <button
          ...
          <!-- Keybind indicator -->
          <span
            class="absolute bottom-0.5 right-0.5 text-[8px] sm:text-[10px] text-muted-foreground font-mono">
            {{ slot.keybind }}
          </span>
        </button>
      </div>
