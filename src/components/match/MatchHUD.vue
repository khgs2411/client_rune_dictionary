...
const playerName = computed(() => matchStore.gameState.player?.name ?? 'Player');
const playerLevel = ref(1); // TODO: Add level to participant interface if needed
const playerHp = computed(() => matchStore.gameState.player?.health ?? 0);
const playerMaxHp = computed(() => matchStore.gameState.player?.maxHealth ?? 1);
const playerMp = ref(10); // TODO: Add MP to participant interface if needed
const playerMaxMp = ref(10);
const playerAtbProgress = computed(() => matchStore.gameState.player?.readiness ?? 0);

// Enemy data from matchStore.gameState.npc (it's a Ref<I_NPCParticipant>)
const enemyName = computed(() => matchStore.gameState.npc?.name ?? 'Enemy');
const enemyLevel = ref(1); // TODO: Add level to participant interface if needed
const enemyHp = computed(() => matchStore.gameState.npc?.health ?? 0);
const enemyMaxHp = computed(() => matchStore.gameState.npc?.maxHealth ?? 1);
