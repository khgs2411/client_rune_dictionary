<template>
	<Teleport to="body">
		<div
			v-if="settings.debug.showWebSocketDebugger"
			...
			@scroll.prevent.stop
			:class="['fixed bg-background/90 backdrop-blur-sm border rounded-lg shadow-lg overflow-hidden flex flex-col z-40 transition-shadow', isResizing ? 'border-primary shadow-xl' : 'border-border']">
			<!-- Header (Draggable Handle) -->
			<div ref="handle" class="flex items-center justify-between px-4 py-2 border-b border-border bg-background/50 cursor-move select-none">
			...
			<div ref="eventListRef" class="flex-1 overflow-y-auto p-2 space-y-1" @wheel.stop>
				<div v-if="events.length === 0" class="text-center text-sm text-muted-foreground py-8">No events yet. Waiting for WebSocket activity...</div>
				<div v-for="(event, index) in events" :key="index" class="text-xs font-mono p-2 rounded bg-muted/50 hover:bg-muted transition-colors cursor-pointer" @click="toggleExpanded(index)">
					<div class="flex items-start justify-between gap-2 mb-1">
						<div class="flex items-center gap-1 flex-1 min-w-0">
							<Icon :icon="expandedEvents.has(index) ? 'radix-icons:chevron-down' : 'radix-icons:chevron-right'" class="h-3 w-3 flex-shrink-0" />
			...
			<!-- Footer Stats -->
			<div class="px-4 py-2 border-t border-border bg-background/50 flex items-center justify-between text-xs text-muted-foreground">
				<span>{{ events.length }} events</span>
				<span class="text-[10px]">{{ size.width }}x{{ size.height }}</span>
			</div>

			<!-- Resize Handle (bottom-right corner) -->
			<div ref="resizeHandle" class="absolute bottom-0 right-0 w-4 h-4 cursor-se-resize opacity-50 hover:opacity-100 transition-opacity">
